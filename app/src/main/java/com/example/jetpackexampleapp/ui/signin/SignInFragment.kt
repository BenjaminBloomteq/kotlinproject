...
class SignInFragment : Fragment() {
    ...
    private fun onSignInBtnClicked() {
        if (validateFields()) {
            if (viewModel.validateUserExists(usernameLoginEditText.text.toString(), passwordLoginEditText.text.toString())) {
    ...
    private fun toSignUp() {
        ...
        activity?.supportFragmentManager?.beginTransaction()?.replace(R.id.fragmentFrameLayout, RegisterFragment(), "registerFragment")?.commit()
    }
}